...
* Changed Navbar styling and layout so that user has more functionality when they log in. The only thing they can see now is the Sign up and log in option until they sign up and sign in.
* Added some styling to the Navbar from Bootstrap, the Login and Sign up page. 
* Created CarouselComponent.jsx and css styling sheet.
* Added Carousel to the Home.jsx page and center aligned quotes. 
* Completed my portion of the README.md main and api integrations page.

Ramesh and I worked together again today and made so much progress!! As we felt time was going by too fast, we felt a bit of stress but we handled it so well as a team! There just wasn't enough time to experiment as well as get the issues we needed to finish done before we reconvened as team for our final wrap up so we decided to stay focus on accomplishing our Issues and if we had some extra time, we experimented a little bit. We had to do so much research on how to implement the Carousel Component correctly and we found that there were so many options. We found that React had a 'slick' carousel from their library and that took us so many tries because since it comes with a default library, we had to create a css sheet to style the carousel's arrows how we needed them. One of the arrows is still messing up a little bit but I think it's because there is a styling naming problem thats conflicting with the default library. One arrow works but the other doesn't, which is the weird part. 

We found that Bootstrap had a Carousel that we almost went with if Ramesh hadn't gotten the React slick one to work. We also had to go to the Bootstrap docs for the navbar because we wanted to make the toggle bar have different option if the user was logged vs if they weren't. We found that if you had a certain version  and found that there were different versions of Bootstrap and how if we were using version 5, we had to make sure we were using the "Navigate" component instead of "Redirect" but I noticed that we were using 4. Ramesh and I decided to not go with the toggle bar option because our application does not have too many links so it wasn't necessary. Most of our afternoon after break consisted of messing with CSS. We tried how to size css the image container, we were able to center the quotes and make the letter styling italic I think is the name. Overall I'm really satisfied with how much we got done today and I hope my teammate was as well. 
...
* Adding final contributions from the rest of the team to the README.md.
* Deconstructing the CarouselComponent.jsx and building the Carousel feature completely using bootstrap. 
* Fixing some final console errors with the navbar not being in place.
* Adding CSS styling to the Create a Check-in, Calendar Details, Edit and Home Page.


Today was definitely the most fun day of the whole 4 weeks. Ramesh and I had clear goals of what we had to work on and even if we didn't know how we were going to accomplish the task, we still worked as a team to figure it out. We are both veterans so I would say we work great under pressure and we know how to manage stress effectively. We had to completely scarp the code we had worked on yesterday using slick because Riley said he was going to be looking for console errors and we were receiving a currentSlide error and we couldn't not figure out why or how to fix it. The react slick documentation was not helpful whatsover because it did not cover any od the things that we needed. No even ChatGPT had a clue of what that error was. That is why we resulted into changing gears and doing something tht we were comfortable with just for the sake of the grade. 

Ramesh and I wanted to keep exploring with slick but there wasn't enough time to do so. After lunch, we came together as a team to discuss what we had been working on and the progress we had made and when the team noticed that the Navbar would not stay at the top of the page, they decided it was better to archive the code we had worked on previously and go back to the slick carousel, which was giving us errors. I had slightly different perspective and I suggested trying out this code, but also acknowledged that if it didn't work, reverting back to the slick one might be the better option. After consulting with the SEIRS, they explained how its better to not show errors in the console at all and proceeded to help me correct the navbar issue by changing a 1 CSS class name. We realized that the issue was in App.jsx and Home.jsx. We had 2 CSS classes that were both called container and were performing different tasks. One was supposed to create to keep the app a certain size and the carousel container was supposed to keep the carousel a certain size and in a center of the page. It was a relief knowing that the fix was actually very simple and that we only needed to change a CSS class name instead of getting rid of all the work done before lunch after starting it from scratch.

Being my final journal entry, this feels unreal. We have come to the end of our Module 3 Project and overall, I am very proud of my team. We havel worked very hard and contributed so much during these last four weeks collaborating as a team and during these 4-5 months we have spent at Hack Reactor learning such a . As we head towards our last Final Assessment, I hope we have all learned from this experience in one way or another and I wish everyone good luck on their assessments and the job hunting process. 
